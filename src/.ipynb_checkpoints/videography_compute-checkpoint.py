...
def classify_motion_with_hmm(motion_energy, smoothed_motion_energy, n_states=2, n_iter=10000):
    """
    Classify motion into discrete states using Gaussian HMM (other method)

    Parameters:
        motion_energy (np.ndarray): Raw motion energy (1D array).
        smoothed_motion_energy (np.ndarray): Smoothed motion energy (1D array), used for prediction.
        n_states (int): Number of hidden states to fit in the HMM. Default is 2.
        n_iter (int): Maximum number of iterations for the HMM fitting. Default is 10000.

    Returns:
        bin_motion_hmm (np.ndarray): Binary array where 1 indicates the 'active' motion state.
        states (np.ndarray): The sequence of HMM-inferred states.
        idx_active_state (int): The state index associated with high motion energ (active).
    """
    from hmmlearn import hmm
    import numpy as np

    model = hmm.GaussianHMM(n_components=n_states, covariance_type="full", n_iter=n_iter)
    model.fit(motion_energy.reshape(-1, 1))
    states = model.predict(smoothed_motion_energy.reshape(-1, 1))

    # determine which state is "active" (high motion energy)
    state_means = [np.mean(motion_energy[states == i]) for i in range(n_states)]
    idx_active_state = np.argmax(state_means)

    # 1 if active, 0 if rest
    bin_motion_hmm = (states == idx_active_state).astype(int)

    return bin_motion_hmm, states, idx_active_state